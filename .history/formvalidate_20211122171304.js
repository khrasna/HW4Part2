//created by Karsten Hrasna. email Karsten_Hrasna@student.uml.edu. Uses jquery to create and display multiplication tables after verification
var tabs;
...
$(function () {
    ...
    $.validator.addMethod("checkval", function (value, element) {
        if (value == "") {
            return true; // making sure user cannot delete tab 1
        } else {
    ...
    $("#FormInput").validate({
        //this makes it so the validator updates the error messages whenever new input is typed. 
        //learned about onkeyup at https://www.sitepoint.com/jquery-validation-validate-form-submit/
        rules: {
            MultiplierStart: {
                range: [-50, 50],
    ...
    $(".refresh").on("input", function () { //display table updating with new input in tab 1
        $("#multTable").remove();
        if ($("#FormInput").valid()) {
            processHere();
        }
...
function processHere() { //same as process except this is a dynamic table which is updated with change of input and displayed in tab 1
